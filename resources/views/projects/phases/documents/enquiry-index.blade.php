...
<div class="container-fluid py-4">
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb bg-light p-3 rounded shadow-sm">
            ...
            <li class="breadcrumb-item">
                <a href="{{ route('enquiries.files', $enquiry) }}" class="text-decoration-none">
                    <i class="bi bi-folder me-1"></i>
                    {{ Str::limit($enquiry->project_name, 30) }}
                </a>
            </li>
            <li class="breadcrumb-item">
                <a href="{{ route('enquiries.files.design-concept', $enquiry) }}" class="text-decoration-none">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            ...
                            <p class="mb-0 opacity-75 text-black">
                                ...
                                Enquiry: {{ $enquiry->project_name }} ({{ $enquiry->formatted_id }})
                            </p>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="{{ route('enquiries.files.design-concept', $enquiry) }}" class="btn btn-outline-light btn-sm me-2">
                            ...
                            <a href="{{ route('enquiries.phases.documents.bulk-download', [$enquiry, $phase]) }}" class="btn btn-success btn-sm">
            ...
            <div class="card-body text-center">
                ...
                <div class="document-actions mt-3">
                    <div class="d-flex justify-content-center gap-2">
                        <a href="{{ route('enquiries.phases.documents.download', [$enquiry, $phase, $document]) }}" 
                        ...
                        <button type="button" class="btn btn-info btn-sm" 
                                onclick="previewDocument({{ $document->id }}, '{{ $document->original_filename }}', '{{ $document->document_type }}', '{{ $document->file_extension }}', '{{ route('enquiries.phases.documents.preview', [$enquiry, $phase, $document]) }}', '{{ $document->description }}', '{{ $document->uploader->name }}', '{{ $document->created_at->format('M d, Y H:i') }}', '{{ $document->file_size_human }}', '{{ route('enquiries.phases.documents.download', [$enquiry, $phase, $document]) }}')"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    uploadForm.addEventListener('submit', function(e) {
        ...
        fetch('{{ route("enquiries.phases.documents.store", [$enquiry, $phase]) }}', {
            method: 'POST',
            body: formData,
            headers: {
...
function deleteDocument(documentId, filename) {
    ...
    fetch(`{{ route('enquiries.phases.documents.destroy', [$enquiry, $phase, ':id']) }}`.replace(':id', documentId), {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
            'X-Requested-With': 'XMLHttpRequest'
